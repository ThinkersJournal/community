 ...
 *
 * Forwards an incoming WS upgrade to the `api` Worker's `/notifications/ws`
 * over the `API` Service Binding and hands the resulting `101` + `webSocket`
 * back to the browser, so `api` can auth the session (Cookie) and
 * origin-check (Origin) the socket itself.
 *
