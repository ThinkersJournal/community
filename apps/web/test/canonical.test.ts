...
describe("canonical URLs", () => {
  it("is an absolute https origin with no trailing slash", () => {
    ...
    expect(CANONICAL_ORIGIN).toBe("https://community.thinkersjournal.com");
    expect(CANONICAL_ORIGIN.endsWith("/")).toBe(false);
  });

  it("builds a profile URL with the @ prefix", () => {
    expect(profileUrl("ada")).toBe("https://community.thinkersjournal.com/@ada");
  });

  it("builds a post URL under the profile", () => {
    expect(postUrl("ada", "hello-world")).toBe("https://community.thinkersjournal.com/@ada/hello-world");
  });
  ...
  it("percent-encodes a slug that would otherwise escape the path", () => {
    ...
    expect(postUrl("ada", "a/../../evil")).toBe("https://community.thinkersjournal.com/@ada/a%2F..%2F..%2Fevil");
  });

  it("percent-encodes a username", () => {
    expect(profileUrl("a b")).toBe("https://community.thinkersjournal.com/@a%20b");
  });
