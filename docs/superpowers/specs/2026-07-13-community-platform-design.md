...
>
> **The enforcement gap is issue #35** (status column + refusal at login). ⚠️ **The capability has LANDED on `feat/issue-35-account-status`: migration `0015` adds `suspended_until`/`disabled_at`/`disabled_reason` to `users`, and `apps/api/src/auth/account-status.ts`'s `isBarred` is consulted by `apps/api/src/routes/login.ts` after the password verify.** What remains open: nothing yet WRITES those columns (today only a human running raw SQL can), and the mutating-pipeline half of this spec's binding — `docs/superpowers/specs/2026-09-06-m4-moderation-queue-design.md:174` requires BOTH login and the mutating pipeline to refuse a barred user; this slice is login only. `apps/api/test/no-unbacked-ban-claim.node.test.ts` now also ratchets on `isBarred` staying wired in, not just on the claim staying corrected.
